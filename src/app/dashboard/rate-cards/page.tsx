...
export default function RateCardsPage() {
  ...
  const [entitlements, setEntitlements] = useState<any>({
    customRateCardLimit: 50,
    usedRateCards: 0,
    systemRateCardAccess: true,
  });

  // Navigation state: null means seeing companies list
  ...
  // System Card selected cache sync
  useEffect(() => {
  ...
  async function loadAllData() {
    try {
      ...
      if (cardsRes.ok) {
        ...
        setCustomCards(cardJson.customCards || []);
        setSystemCards(cardJson.systemCards || []);
        setEntitlements(cardJson.entitlements);
      }
    } catch (err) {
  ...
  const toggleSystemCardComparison = (id: string) => {
    let updated;
    if (selectedSystemCardIds.includes(id)) {
      updated = selectedSystemCardIds.filter(x => x !== id);
    } else {
      updated = [...selectedSystemCardIds, id];
    }
    setSelectedSystemCardIds(updated);
    localStorage.setItem('selectedSystemRateCards', JSON.stringify(updated));
  };
  ...
  const handleCompanyDelete = async (compId: string, name: string) => {
    if (!confirm(`Warning: Deleting company "${name}" will permanently remove all associated custom rate cards. Continue?`)) return;
    setError('');
    setSuccess('');
    try {
      const res = await fetch(`/api/courier-companies/${compId}`, { method: 'DELETE' });
      if (res.ok) {
        setSuccess('Courier company and nested cards removed.');
  ...
  const handleCardSubmit = async (e: React.FormEvent) => {
    ...
    if (!rateCardName || !serviceName) {
      setError('Please fill in Rate Card Name and Service Name.');
      setSaving(false);
      return;
    }
    ...
    try {
      const url = cardEditing ? `/api/rate-cards/${cardEditId}` : '/api/rate-cards';
      ...
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || 'Failed to save rate card.');

      setSuccess(cardEditing ? 'Rate card updated.' : 'Rate card added successfully.');
  ...
  const handleDuplicateCard = async (cardId: string) => {
    ...
    setSuccess('');
    try {
      const res = await fetch('/api/rate-cards/duplicate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ rateCardId: cardId }),
      });
      if (res.ok) {
        setSuccess('Rate card duplicated.');
        await loadAllData();
      }
    } catch (err: any) {
      setError('Duplication failed.');
    }
  };
  ...
  const toggleCardComparison = async (card: any) => {
    try {
      const res = await fetch(`/api/rate-cards/${card.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ useForComparison: !card.useForComparison }),
      });
      if (res.ok) {
        await loadAllData();
      }
  ...
  const addWeightSlab = () => {
    ...
    // Update region rows prices matrix to accommodate the new slab
  ...
  const updateWeightSlabField = (index: number, field: keyof WeightSlab, val: any) => {
    ...
    // Auto-update display labels when weight or unit or type changes
  ...
  const deleteWeightSlab = (slabId: string) => {
    ...
    // Remove price entries linked to this slab
    const updatedMatrix = regionsMatrix.map(row => ({
      ...row,
  ...
  // Dynamic Regions Matrix modifiers
  ...
  const updateRegionPriceField = (regId: string, slabId: string, val: string) => {
    setRegionsMatrix(regionsMatrix.map(r => {
      ...
      // Ensure the slabId price exists in array
  ...
  const getNestedCardsCount = (compId: string, type: 'Domestic' | 'International') => {
    return customCards.filter(c => c.courierCompanyId === compId && c.serviceType === type).length;
  };
    ...
    return (
      <div className="min-h-[400px] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-500 text-xs font-semibold">Loading rate cards directory...</p>
        </div>
      </div>
    );
  }

  // RENDER LEVEL 1: COURIER COMPANIES VIEW
  if (selectedCompany === null) {
    return (
      <div className="space-y-6 w-full max-w-7xl mx-auto">
        <div className="flex items-center justify-between flex-wrap gap-4 border-b border-slate-200 pb-4">
          <div>
            <h1 className="text-xl font-bold text-[#0F4C3A]">Rate Card Directories</h1>
            <p className="text-xs text-slate-500 mt-1 font-light">
              Create independent Courier Company contracts to configure multiple Domestic/International service rate cards.
            </p>
          </div>
        ...
        {error && (
          <div className="bg-red-50 text-red-650 border border-red-250 p-3.5 rounded-xl text-xs font-semibold flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <span>{error}</span>
        ...
        {companies.length === 0 ? (
          <div className="bg-white border border-slate-200 p-12 text-center rounded-2xl shadow-xs max-w-lg mx-auto">
            ...
            <h3 className="font-extrabold text-slate-800 text-sm">No Courier Companies Found</h3>
            <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1 leading-relaxed font-normal">
              Add courier operators such as DTDC, Blue Dart, or custom logistics carriers first to configure pricing cards under them.
            </p>
          </div>
        ) : (
          <div className={
              ...
              return (
                ...
                >
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <h3 className="font-extrabold text-slate-900 text-base tracking-tight truncate flex items-center gap-2">
                            <span>{comp.name}</span>
                            {comp.userId === null && (
                              <span className="bg-slate-100 text-slate-650 text-[9px] font-black uppercase px-2 py-0.5 rounded-full border border-slate-200 shrink-0">
                                SYSTEM
                              </span>
                            )}
                          </h3>
                    ...
                    {/* Secondary Information: Domestic & International Card Counts */}
                    <div className="grid grid-cols-2 gap-3 pt-1">
                      <div className="bg-slate-50/80 p-3 rounded-xl border border-slate-200/60">
                        ...
                        <strong className="text-slate-900 text-sm font-black mt-0.5 block">{domCount} {domCount === 1 ? 'Card' : 'Cards'}</strong>
                      </div>
                      <div className="bg-slate-50/80 p-3 rounded-xl border border-slate-200/60">
                        <span className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider">International</span>
                        <strong className="text-slate-900 text-sm font-black mt-0.5 block">{intCount} {intCount === 1 ? 'Card' : 'Cards'}</strong>
                      </div>
                    </div>
                  </div>
        ...
        <Modal
          ...
          title={companyEditing ? 'EDIT COURIER COMPANY' : 'ADD COURIER COMPANY'}
          size="sm"
        >
            ...
            <div className="flex justify-end gap-3 border-t border-slate-100 pt-4 mt-2">
              ...
              >
                {saving ? (
                  ...
                  <span>Save Courier Company</span>
                )}
              </button>
            </div>
          </form>
  ...
  // RENDER LEVEL 2: DETAILED NESTED TABS VIEW
  const isSystemCompany = selectedCompany.userId === null;
  const filteredCustomCards = customCards.filter(c => c.courierCompanyId === selectedCompany.id && c.serviceType === subTab);
  ...
  return (
    <div className="space-y-6 w-full max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 pb-4 flex-wrap gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-bold text-[#0F4C3A]">{selectedCompany.name}</h1>
              {isSystemCompany && (
                <span className="bg-emerald-50 text-emerald-800 text-[8px] font-black tracking-wider uppercase px-2 py-0.5 rounded-full border border-emerald-150">
                  SYSTEM OPERATOR
                </span>
              )}
            </div>
            <div className="flex gap-4 flex-wrap mt-0.5">
              {selectedCompany.trackingUrl && selectedCompany.trackingUrl.trim() !== '' && (
                <a
                  href={selectedCompany.trackingUrl}
                  target="_blank"
                  rel="noreferrer"
                  className="text-[10px] text-[#1E8262] hover:underline flex items-center gap-0.5"
                >
                  Official Tracking Portal
                  <ExternalLink className="w-2.5 h-2.5" />
                </a>
              )}
              {selectedCompany.pincodeServiceabilityUrl && selectedCompany.pincodeServiceabilityUrl.trim() !== '' && (
                <a
                  href={selectedCompany.pincodeServiceabilityUrl}
                  target="_blank"
                  rel="noreferrer"
                  className="text-[10px] text-[#1E8262] hover:underline flex items-center gap-0.5"
                >
                  Pincode Serviceability Portal
                  <ExternalLink className="w-2.5 h-2.5" />
                </a>
              )}
            </div>
        ...
        {!isSystemCompany && subTab !== 'International' && (
          <button
            onClick={openAddCard}
            className="bg-[#1E8262] hover:bg-[#0F4C3A] text-white py-2 px-4 rounded-xl text-xs font-bold transition flex items-center gap-1.5 shadow-md"
          >
            <Plus className="w-3.5 h-3.5" />
            Add {subTab} Rate Card
          </button>
        )}
      </div>
      ...
      {error && (
        <div className="bg-red-50 text-red-655 border border-red-255 p-3.5 rounded-xl text-xs font-semibold flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <span>{error}</span>
          {error.toLowerCase().includes('subscribe') && (
            <a
              href="/dashboard/subscriptions"
      ...
      {/* Sub Tabs: Domestic vs. International */}
      ...
      {subTab === 'International' ? (
        <InternationalRateCardManager
          ...
          rateCards={[...customCards, ...systemCards]}
          companies={companies}
        ...
        <div className="space-y-6">
          <div>
            <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Custom Rate Contracts</h2>
            {filteredCustomCards.length === 0 ? (
              <div className="bg-white border border-slate-200 p-8 text-center rounded-2xl shadow-sm text-slate-405 text-xs">
                No custom {subTab} rate cards defined for this operator.
              </div>
          ) : (
              ...
              <table className="w-full text-left border-collapse min-w-[800px]">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 font-bold p-3">
                    ...
                    <th className="p-3">Rate Card Name</th>
                ...
                <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
                  {filteredCustomCards.map((card) => (
                    <tr key={card.id} className="hover:bg-slate-50/50">
                        ...
                        <button
                          ...
                          className={`px-2 py-0.5 rounded-full text-[9px] font-bold border transition ${
                            card.active
                              ...
                              : 'bg-red-50/50 text-red-650 border-red-150 hover:bg-red-50'
                          }`}
                        >
                      ...
                      <td className="p-3 text-center">
                        <button
                          onClick={() => toggleCardComparison(card)}
                          className={`px-2 py-0.5 rounded-full text-[9px] font-bold border transition ${
                            card.useForComparison
                              ? 'bg-teal-50 text-teal-700 border-teal-200 hover:bg-teal-100'
                              : 'bg-slate-100 text-slate-500 border-slate-200 hover:bg-slate-155'
                          }`}
                        >
                          {card.useForComparison ? 'COMPARE ON' : 'COMPARE OFF'}
                        </button>
                      </td>
                      <td className="p-3 text-center flex items-center justify-center gap-1.5">
                        ...
                        <button
                          ...
                          className="p-1 hover:bg-slate-100 rounded text-slate-505"
                          title="Duplicate"
                        >
        ...
        {/* System Operator Templates */}
        {filteredSystemTemplates.length > 0 && (
          <div>
            <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">System Contract Templates</h2>
            <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-x-auto text-xs">
              <table className="w-full text-left border-collapse min-w-[800px]">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200 text-slate-505 font-bold p-3">
                    ...
                    <th className="p-3 text-center">Threshold</th>
                    <th className="p-3 text-center">Status</th>
                    <th className="p-3 text-center">Comparison Participation</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
                  {filteredSystemTemplates.map((card) => {
                    const isComparing = selectedSystemCardIds.includes(card.id);
                    return (
                      <tr key={card.id} className="hover:bg-slate-50/50">
                        <td className="p-3 font-bold text-slate-800">{card.serviceName}</td>
                        <td className="p-3 text-slate-600 font-semibold">{card.rateCardName}</td>
                        <td className="p-3 text-slate-550 uppercase">{card.pricingModel === 'SLAB' ? 'Weight Slab' : (card.pricingModel === 'LTL_PTL' ? 'LTL/PTL' : 'Per KG')}</td>
                        <td className="p-3 text-center font-mono">{card.minimumWeight} KG</td>
                        <td className="p-3 text-center font-mono">{card.cargoThreshold} KG</td>
                        <td className="p-3 text-center">
                          <span className="bg-emerald-50 text-emerald-800 text-[9px] font-bold border border-emerald-100 px-2 py-0.5 rounded-full">
                            SYSTEM ACTIVE
                          </span>
                        </td>
                        <td className="p-3 text-center">
                          <button
                            onClick={() => toggleSystemCardComparison(card.id)}
                            className={`px-2.5 py-0.5 rounded-full text-[9px] font-bold border transition ${
                              isComparing
                                ? 'bg-teal-50 text-teal-700 border-teal-200 hover:bg-teal-100'
                                : 'bg-slate-105 text-slate-505 border-slate-200 hover:bg-slate-150'
                            }`}
                          >
                            {isComparing ? 'COMPARE ON' : 'COMPARE OFF'}
                          </button>
                        </td>
                      </tr>
                    );
      ...
      <Modal
        ...
        title={cardEditing ? `EDIT RATE CARD: ${rateCardName || 'UNTITLED'}` : `ADD ${subTab.toUpperCase()} RATE CARD`}
        size="workspace"
        noPadding
        headerExtra={
          <span className="hidden sm:inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold bg-emerald-50 text-[#0F4C3A] border border-emerald-200">
            <Sparkles className="w-3 h-3 text-[#1E8262]" /> Rate Card Workspace
          </span>
        }
        footer={
              ...
              >
                {saving ? (
                  ...
                  <>
                    ...
                    <span>{cardEditing ? 'Save Rate Card Changes' : 'Save Rate Card'}</span>
                  </>
                )}
              </button>
            </div>
            ...
            <div className="bg-white border border-slate-200/80 rounded-xl p-5 shadow-sm space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-4">
                <div className="xl:col-span-2">
                  <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Rate Card Name</label>
                  <input
                    type="text"
