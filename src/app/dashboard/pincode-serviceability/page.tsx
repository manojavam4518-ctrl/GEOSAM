...
import { MapPin, Loader2, ExternalLink, Plus, X, Trash2, Upload } from 'lucide-react';

const STORAGE_KEY = 'geo_custom_pincode_couriers';
const DELETED_STORAGE_KEY = 'geo_deleted_pincode_couriers';
...
export default function PincodePage() {
  ...
  const [serviceabilityUrl, setServiceabilityUrl] = useState('');
    ...
    async function loadCouriers() {
      try {
        ...
        try {
          ...
          console.error('Failed to read custom couriers from localStorage', e);
        }

        // Load deleted couriers list (supports persisting deletion of default and custom couriers)
          ...
          (custom) =>
            ...
            !baseCouriers.some(
              (base) =>
                ...
                base.pincodeServiceabilityUrl?.trim().toLowerCase() ===
                  custom.pincodeServiceabilityUrl?.trim().toLowerCase()
            )
        );
  ...
  const handleAddCourier = (e: React.FormEvent) => {
    ...
    const trimmedUrl = serviceabilityUrl.trim();
    ...
    if (!trimmedUrl) {
      setFormError('Please enter the serviceability portal URL.');
      return;
    }
    ...
    try {
      ...
      setFormError('Please enter a valid URL (e.g. https://example.com/pincode)');
      return;
    }

    // Prevent duplicate entries where the same name AND url already exist
    const isDuplicate = couriers.some(
      (c) =>
        c.name?.trim().toLowerCase() === trimmedName.toLowerCase() &&
        c.pincodeServiceabilityUrl?.trim().toLowerCase() === validUrl.toLowerCase()
    );

    if (isDuplicate) {
      setFormError('A courier with this company name and portal URL already exists.');
      return;
    }
    ...
    const newCourier = {
      ...
      name: trimmedName,
      pincodeServiceabilityUrl: validUrl,
    ...
    try {
      ...
      console.error('Failed to persist custom courier to localStorage', err);
    }

    // Reset form and close modal
    setCourierName('');
    setServiceabilityUrl('');
    setIconPreview(null);
  ...
  const handleConfirmDelete = () => {
    ...
    // Persist deletion so it doesn't return after refresh (works for both default and custom couriers)
    try {
      const delStored = localStorage.getItem(DELETED_STORAGE_KEY);
    ...
    // Also remove from custom couriers storage if it was stored there
    try {
      ...
      console.error('Failed to remove custom courier from localStorage', err);
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Select a logistics partner to check their official zip/pincode service coverage portals
          </p>
        </div>
          ...
          {couriers.map((courier) => (
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                {/* Uploaded Icon or Default Letter Badge */}
                {courier.logoUrl ? (
                  ...
                  <div className="w-10 h-10 rounded bg-[#E8F5E9] text-[#0F4C3A] flex items-center justify-center font-bold text-sm border border-emerald-200 shrink-0">
                    {courier.name ? courier.name[0].toUpperCase() : 'C'}
                  </div>
                )}
              ...
              {/* Action Icons: External Link + Delete/Trash */}
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-md overflow-hidden animate-in fade-in duration-150">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              <h2 className="text-sm font-bold text-[#0F4C3A]">Add Courier Serviceability Portal</h2>
              <button
                type="button"
                onClick={() => {
                  setIsModalOpen(false);
            ...
            <form onSubmit={handleAddCourier} className="p-5 space-y-4">
              ...
              {/* 2. Serviceability Portal URL */}
              <div className="space-y-1.5">
                <label className="block text-xs font-semibold text-slate-700">
                  Serviceability Portal URL <span className="text-red-500">*</span>
                </label>
                <input
                  type="url"
                  value={serviceabilityUrl}
                  onChange={(e) => setServiceabilityUrl(e.target.value)}
                  placeholder="https://example.com/pincode"
                  className="w-full px-3 py-2 text-xs border border-slate-200 rounded-xl focus:outline-hidden focus:ring-2 focus:ring-[#1E8262]/20 focus:border-[#1E8262] transition text-slate-800 placeholder-slate-400"
                />
              </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <input
                      ref={fileInputRef}
                      type="file"
                      id="courier-icon-upload"
                      accept=".png,.jpg,.jpeg,.webp,image/png,image/jpeg,image/webp"
                      onChange={handleFileChange}
                      className="hidden"
                    />
                    <label
                      htmlFor="courier-icon-upload"
